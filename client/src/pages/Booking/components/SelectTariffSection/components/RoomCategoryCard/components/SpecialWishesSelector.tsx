...
export const SpecialWishesSelector = memo(
  (props: Props) => {
    const {} = props;
    const dispatch = useAppDispatch();
    const { updateBookingDraft, bookingProgressCurrentStep } =
      useContext(BookingContext);
    const { currentBooking, currentRoomCategory: roomCategory } =
      useAppSelector((state) => state.bookings);
    const { roomFeatures } = useAppSelector((state) => state.roomFeatures);
    const { roomBedVariants } = useAppSelector(
      (state) => state.roomBedVariants
    );
    const { viewsFromRoomWindow } = useAppSelector(
      (state) => state.viewsFromRoomWindow
    );
    const [bedSpecialWish, setBedSpecialWish] = useState<
      ToogleButtonModeType[]
    >([]);
    const [viewsFromWindowSpecialWish, setViewsFromWindowSpecialWish] =
      useState<ToogleButtonModeType[]>([]);
